...
def calculate_significant_scores(df, p_value_threshold=0.05):
    ...
    p_value_columns = [
        "A_frequency_p_value_Wilcoxon",
        "T_frequency_p_value_Wilcoxon",
        "G_frequency_p_value_Wilcoxon",
        "C_frequency_p_value_Wilcoxon",
    ]
...
def main():
    ...
    output_combined = os.path.join(
        args.output_dir, f"{prefix}_gene_scores_combined_old.tsv"
    )
    ...
    output_individual = os.path.join(
        args.output_dir, f"{prefix}_gene_scores_individual_old.tsv"
    )
    ...
    output_overlapping = os.path.join(
        args.output_dir, f"{prefix}_gene_scores_overlapping_old.tsv"
    )
