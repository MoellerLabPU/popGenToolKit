...
from alleleflux.scripts.analysis.ani.strain_turnover import call_transitions
...
def _pair_row(s1, s2, subj, t1, t2, popani, pgc, conani=None, group="40"):
    ...
    return {
        ...
        "replicate_2": "1",
        "popANI": popani,
        "conANI": popani if conani is None else conani,
        "percent_genome_compared": pgc,
        "compared_bases_count": int(pgc * 4_000_000),
        "min_cov": 5,
    }
...
class TestCallTransitions(unittest.TestCase):
    ...
    def test_four_backgrounds(self):
        got = self._call(
            [
                _pair_row(
                    "A1", "A2", "m1", "5mo", "22mo", 0.9999990, 0.8, conani=0.9995
                ),  # stable
                _pair_row(
                    "B1", "B2", "m2", "5mo", "22mo", 0.9999950, 0.8, conani=0.9985
                ),  # dominant only
                _pair_row(
                    "C1", "C2", "m3", "5mo", "22mo", 0.9999800, 0.8, conani=0.9995
                ),  # replacement only
                _pair_row(
                    "D1", "D2", "m4", "5mo", "22mo", 0.9900000, 0.8, conani=0.9800
                ),  # both
            ]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            list(got["dominant_strain_change"]), [False, True, False, True]
        )
    ...
    def test_threshold_is_strict_less_than(self):
        ...
        got = self._call(
            [_pair_row("A1", "A2", "m1", "5mo", "22mo", 0.99999, 0.8, conani=0.999)]
        )
    ...
    def test_min_compared_is_configurable(self):
        ...
        self.assertEqual(
            self._call([row]).iloc[0]["background"], "undetermined"
        )  # 5% < 10%
        self.assertEqual(
            self._call([row], min_compared=0.05).iloc[0]["background"],
            "strain_replacement+dominant_strain_change",
        )  # 5% >= 5%
    ...
    def test_non_transition_and_cross_subject_rows_are_excluded(self):
        rows = [
            _pair_row(
                "A1", "A2", "m1", "5mo", "28mo", 0.9999990, 0.8
            ),  # not a configured transition
            _pair_row(
                "A1", "B1", "m1", "5mo", "22mo", 0.9999990, 0.8
            ),  # two different mice
        ]
    ...
    def test_rejects_degenerate_transition(self):
        with self.assertRaises(ValueError):
            call_transitions(
                pd.DataFrame([_pair_row("A", "B", "m", "5mo", "22mo", 1.0, 1.0)]),
                [("5mo", "5mo")],
                0.1,
                0.99999,
                0.999,
...
def _write_profile(directory, sample, mag, rows):
    ...
    with gzip.open(
        os.path.join(sample_dir, f"{sample}_{mag}_profiled.tsv.gz"), "wt"
    ) as handle:
        handle.write(PROFILE_HEADER)
        for contig, pos, ref, a, c, g, t, gene in rows:
            handle.write(
                f"{contig}\t{pos}\t{ref}\t{a + c + g + t}\t{a}\t{c}\t{g}\t{t}\t0\t{gene}\n"
            )
...
class TestStrainTurnoverCLI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        _write_profile(
            ...
            "S4",
            self.MAG,
            [("c1", p, "A", 0, 0, 20, 0, "g1") for p in range(6)],
        )
        ...
        pd.DataFrame(
            {
                ...
                "subjectID": ["m1", "m2", "m1", "m2"],
                "replicate": ["r1", "r2", "r1", "r2"],
                "time": ["pre", "pre", "end", "end"],
                "genome_size": [6] * 4,
                "breadth": [1.0] * 4,
                "coverage_threshold_passed": [True] * 4,
            }
    ...
    def _run(self, *extra):
        return subprocess.run(
            [
                ...
                self.MAG,
                "--pair_table",
                os.path.join(self.pairwise, f"{self.MAG}_pairwise_ani.tsv"),
                "--output_dir",
                self.out,
                "--transitions",
                "pre:end",
    ...
    def _read(self, suffix, **kw):
        return pd.read_csv(
            os.path.join(self.out, f"{self.MAG}{suffix}"), sep="\t", **kw
        )
    ...
    def test_turnover_table_has_one_row_per_mouse_with_both_verdicts(self):
        ...
        self.assertEqual(
            (m1.sample_t1, m1.sample_t2, m1.transition), ("S1", "S3", "pre_end")
        )
        ...
        self.assertEqual(
            (
                float(m1.min_compared),
                float(m1.pop_threshold),
                float(m1.con_threshold),
                int(m1.min_cov),
            ),
            (0.1, 0.99999, 0.999, 5),
        )
    ...
    def test_rollup_keeps_groups_separate(self):
        ...
        self.assertEqual(
            (int(fat.n_pairs), int(fat.n_stable), int(fat.n_both)), (1, 1, 0)
        )
        self.assertAlmostEqual(float(fat.median_percent_genome_compared), 5 / 6)
        self.assertEqual(
            (int(control.n_pairs), int(control.n_both), int(control.n_stable)),
            (1, 1, 0),
        )
        ...
        for row in (fat, control):
            self.assertEqual(
                int(
                    row.n_undetermined
                    + row.n_stable
                    + row.n_strain_replacement
                    + row.n_dominant_strain_change
                    + row.n_both
                ),
                int(row.n_pairs),
            )
