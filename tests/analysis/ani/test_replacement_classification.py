...
def _call_row(mag, mouse, group, transition, replacement, dominant, replicate=None):
    """One Task-3 output row.  ``None`` for a flag means undetermined (pd.NA)."""
    return {
        "MAG_ID": mag, "subjectID_1": mouse, "subjectID_2": mouse,
        "group_1": group, "group_2": group,
        "replicate_1": replicate or mouse, "replicate_2": replicate or mouse,
...
class TestMouseBlock(unittest.TestCase):
    ...
    def test_mixed_group_within_a_pair_raises(self):
        rows = [_call_row("MAG_A", "m1", "40", "5mo_22mo", True, False)]
        rows[0]["group_2"] = "20"
        with self.assertRaises(ValueError):
            classify_mags(_frame(rows))
